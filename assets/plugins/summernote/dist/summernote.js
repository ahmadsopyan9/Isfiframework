...
var ImageDialog_ImageDialog = /*#__PURE__*/function () {
  ...
  ImageDialog_createClass(ImageDialog, [{
    ...
    value: function initialize() {
      ...
      var $container = this.options.dialogsInBody ? this.$body : this.options.container;
      var body = ['<div class="form-group note-form-group note-group-select-from-files">', '<label for="note-dialog-image-file-' + this.options.id + '" class="note-form-label">' + this.lang.image.selectFromFiles + '</label>', '<input id="note-dialog-image-file-' + this.options.id + '" class="note-image-input form-control-file note-form-control note-input" ', ' type="file" name="files" accept="image/*" multiple="multiple"/>', imageLimitation, '</div>', '<div class="form-group note-group-image-url">', '<label for="note-dialog-image-url-' + this.options.id + '" class="note-form-label">' + this.lang.image.url + '</label>', '<input id="note-dialog-image-url-' + this.options.id + '" class="note-image-url form-control note-form-control note-input" type="text"/>', '</div><div class="box-image-custom row p-4" style="height:400px;overflow-y: auto;border:1px solid #eaeaea;"></div>'].join('');
      ...
      // custom script
      $.ajax({
        url: base_url+"api/load-all-images",
        type: 'POST',
        contentType: 'application/json',
        data: JSON.stringify({}),
        success:function(res){
          if(res.success){
            $(".box-image-custom").html('');
            let _html = '';
            const data = res.data;
            $.each(data,function(i,v){
              _html += `
                <div class="img-modal-custom-selected col-xs-4 col-sm-2 pl-1 pr-1 mb-2">
                  <img src="${v.url}" style="width:100%;height:100px;border:2px solid #ccc;">
                </div>
              `;
            });
            $(".box-image-custom").html(_html);
            $(".img-modal-custom-selected img").on("click",function(e){
              $(".text_editor").summernote("insertImage", e.currentTarget.src);
